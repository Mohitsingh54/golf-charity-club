...
function renderHeaderStats() {
  ...
  els.headlineStats.innerHTML = `
    ...
    <div class="metric-card">
      ...
      <strong>$${charityTotal.toLocaleString()}</strong>
  ...
  els.latestWinningNumbers.innerHTML = (latestDraw?.winningNumbers || [3, 7, 12, 18, 24])
    ...
    .join("");
  els.jackpotSummary.textContent = `Current jackpot pool: $${calculatePrizePool().toLocaleString()} including $${state.jackpotCarryOver.toLocaleString()} rollover.`;
}
    ...
    ? drawHistory.map((draw) => {
        ...
        return `
          <article class="history-item">
            ...
            <p>Payout: $${result.payout.toLocaleString()} | Status: ${titleCase(result.paymentStatus)}</p>
          </article>
        `;
      }).join("")
...
function renderCharityHighlights() {
  const breakdown = state.charities.map((charity) => {
    ...
    const amount = users.reduce((sum, user) => sum + Math.round(user.fee * (user.contributionPercent / 100)), 0);
  ...
  els.charityHighlights.innerHTML = breakdown
    .map(({ charity, amount, supporters }) => `
      <article class="charity-card">
        ...
        <strong>$${amount.toLocaleString()} projected contribution</strong>
      </article>
    `)
    .join("");
}
  ...
  els.prizeBreakdown.innerHTML = prizePercentages
    .map((tier) => `
      <article class="prize-tier">
        ...
        <p>$${Math.round(prizePool * tier.percent).toLocaleString()} ${tier.jackpot ? "with rollover enabled if unclaimed" : "shared equally across verified winners"}</p>
      </article>
    `)
    .join("");
}
  ...
  els.drawResults.innerHTML = `
    ...
    ${winners.map((winner) => `
      <article class="draw-row">
        ...
        <p>Payout $${winner.payout.toLocaleString()} | Verification ${titleCase(winner.verificationStatus)} | Payment ${titleCase(winner.paymentStatus)}${winner.weightedBoost ? " | Score boost applied" : ""}</p>
      </article>
...
function renderAnalytics() {
  ...
  els.analyticsCards.innerHTML = `
    ...
    <article class="analytics-card">
      ...
      <strong>$${calculateCharityTotal().toLocaleString()}</strong>
...
function buildDrawOutcome({ mode, label, winningNumbers, date }) {
  ...
  prizePercentages.forEach((tier) => {
    ...
    const tierAmount = Math.round(prizePool * tier.percent);
    ...
    const perWinner = Math.round(tierAmount / matchingResults.length);
...
function calculatePrizePool() {
  const basePool = getActiveUsers().reduce((sum, user) => sum + Math.round(user.fee * 0.25), 0);
  return basePool + state.jackpotCarryOver;
}

function calculateCharityTotal() {
  return state.users.reduce((sum, user) => sum + Math.round(user.fee * (user.contributionPercent / 100)), 0);
}
